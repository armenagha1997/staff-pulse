...
**После сдачи, по фидбеку от реального использования** (не от тестов —
пользователь заметил проблему при обычном клике по таблице): одиночный клик
по заголовку сортировки был реализован как безусловное присвоение `asc`
(двойной клик — `desc`), буквально по формулировке задания. На практике это
означало, что повторный клик по уже отсортированному по возрастанию столбцу
визуально ничего не делал — пользователь кликал несколько раз, ожидая тоггл
направления (как в большинстве таблиц), и только случайный быстрый повторный
клик, распознанный браузером как `dblclick`, давал видимый эффект. Отсюда
жалоба «иногда нужно трижды кликнуть». Исправление: клик по уже активному
столбцу тоггловит `asc ↔ desc`, клик по другому столбцу — `asc`; двойной
клик по-прежнему надёжно даёт `desc` для любого столбца (см. обновление в
[ADR-004](docs/adr/004-view-mode-and-sort-interaction.md)). Формулировка
задания («двойной клик — обратная») по-прежнему соблюдена буквально — просто
одиночный клик перестал быть no-op.
